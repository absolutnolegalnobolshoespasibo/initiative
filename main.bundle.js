...
var PlayerApi = class {
  ...
  getRole() {
    return __awaiter(this, void 0, void 0, function* () {
      const { role: role2 } = yield this.messageBus.sendAsync("OBR_PLAYER_GET_ROLE", {});
      return role2;
    });
  }
  getMetadata() {
  ...
  hasPermission(permission) {
    return __awaiter(this, void 0, void 0, function* () {
      const role2 = yield this.getRole();
      if (role2 === "GM") {
        return true;
      }
...
function createDefaultState() {
  return {
    phase: "idle",
    // "idle" -> "round" -> "battle" -> ("idle")
    roundStartedAt: null,
    // когда нажали "Начать раунд" (timestamp, мс)
    participants: {},
    // { [itemId]: { ready: bool, readyAt: number|null } }
    order: null,
    // [itemId, ...] — порядок боя после "В бой"
    currentIndex: null,
    // индекс текущего существа в order
    finalInitiative: null
    // { [itemId]: number } — застывшие значения для боя
  };
}
function getCharData(item, roleMap2, itemKey) {
  const raw = item.metadata && item.metadata[itemKey] || {};
  let playerEditable;
  if (typeof raw.playerEditable === "boolean") {
    playerEditable = raw.playerEditable;
  } else {
    playerEditable = roleMap2.get(item.createdUserId) !== "GM";
  }
  return {
    initialInitiative: Number.isFinite(raw.initialInitiative) ? raw.initialInitiative : 0,
    playerEditable
  };
}
...
function startRoundParticipants(characters2) {
  const participants = {};
  for (const item of characters2) {
    participants[item.id] = { ready: false, readyAt: null };
  }
  return participants;
}
function toggleReadyParticipants(state2, itemId, now) {
  const participants = { ...state2.participants };
  const current2 = participants[itemId] || { ready: false, readyAt: null };
  participants[itemId] = current2.ready ? { ready: false, readyAt: null } : { ready: true, readyAt: now };
  return participants;
}
function allReady(characters2, state2) {
  if (characters2.length === 0) return false;
  return characters2.every((c) => state2.participants[c.id]?.ready);
}
function readyCount(characters2, state2) {
  return characters2.filter((c) => state2.participants[c.id]?.ready).length;
}
function rankForBattle(characters2, state2, roleMap2, itemKey, now) {
  const ranked = characters2.map((item) => ({
    id: item.id,
    initiative: computeInitiative(item, state2, roleMap2, itemKey, now),
    readyAt: state2.participants[item.id]?.readyAt ?? now
  }));
  ranked.sort((a, b) => {
...
function nextIndexOrReset(state2) {
  if (!state2.order) return { type: "reset" };
  const nextIndex = state2.currentIndex + 1;
  if (nextIndex >= state2.order.length) {
    return { type: "reset" };
  }
  return { type: "advance", index: nextIndex };
}
function splitByEditable(characters2, roleMap2, itemKey) {
  const npc = [];
  const pc = [];
  for (const item of characters2) {
    const data = getCharData(item, roleMap2, itemKey);
...
var tickHandle = null;
var readyAnimatedAt = /* @__PURE__ */ new Map();
var ANIM_WINDOW_MS = 300;
...
async function resolveRole() {
  if (typeof lib_default.player.role === "string") {
    return lib_default.player.role;
  }
  if (typeof lib_default.player.getRole === "function") {
    try {
      return await lib_default.player.getRole();
    } catch (e) {
...
async function refreshRoleMap() {
  try {
    const others = await lib_default.party.getPlayers();
    const map = /* @__PURE__ */ new Map();
    for (const p of others) {
      map.set(p.id, p.role);
    }
    map.set(lib_default.player.id, role);
    roleMap = map;
  } catch (e) {
...
async function saveState(next) {
  ...
  render();
  try {
    await lib_default.room.setMetadata({ [ROOM_KEY]: state });
  } catch (e) {
...
async function toggleReady(itemId) {
  ...
  if (!state.participants[itemId]?.ready) {
    readyAnimatedAt.set(itemId, Date.now());
  } else {
    readyAnimatedAt.delete(itemId);
  }
...
async function centerOn(itemId) {
  try {
    const [bounds, scale, width, height] = await Promise.all([
      lib_default.scene.items.getItemBounds([itemId]),
      lib_default.viewport.getScale(),
      lib_default.viewport.getWidth(),
      lib_default.viewport.getHeight()
    ]);
    if (!bounds || !bounds.center) return;
    await lib_default.viewport.animateTo({
      scale,
      position: {
...
async function setInitialInitiative(itemId, value) {
  ...
  try {
    await lib_default.scene.items.updateItems([itemId], (items) => {
...
function renderIdle() {
  if (characters.length === 0) {
    return `
      <div class="empty">
        \u041D\u0430 \u0441\u043B\u043E\u0435 \xAB\u041F\u0435\u0440\u0441\u043E\u043D\u0430\u0436\u0438\xBB (Character) \u043F\u043E\u043A\u0430 \u043D\u0435\u0442 \u0442\u043E\u043A\u0435\u043D\u043E\u0432.<br />
  ...
  const rows = characters.map((item) => {
    ...
    const tag = isGM() ? `<button
             class="tag-toggle ${data.playerEditable ? "is-pc" : "is-npc"}"
             data-action="toggle-editable"
             data-id="${item.id}"
             title="${data.playerEditable ? "\u0414\u043E\u0441\u0442\u0443\u043F\u0435\u043D \u0434\u043B\u044F \u0440\u0435\u0434\u0430\u043A\u0442\u0438\u0440\u043E\u0432\u0430\u043D\u0438\u044F \u0438\u0433\u0440\u043E\u043A\u0430\u043C. \u041D\u0430\u0436\u043C\u0438\u0442\u0435, \u0447\u0442\u043E\u0431\u044B \u0441\u0434\u0435\u043B\u0430\u0442\u044C NPC." : "NPC/\u043C\u043E\u043D\u0441\u0442\u0440 \u2014 \u043D\u0435\u0434\u043E\u0441\u0442\u0443\u043F\u0435\u043D \u0434\u043B\u044F \u0440\u0435\u0434\u0430\u043A\u0442\u0438\u0440\u043E\u0432\u0430\u043D\u0438\u044F \u0438\u0433\u0440\u043E\u043A\u0430\u043C. \u041D\u0430\u0436\u043C\u0438\u0442\u0435, \u0447\u0442\u043E\u0431\u044B \u0441\u0434\u0435\u043B\u0430\u0442\u044C PC."}"
...
function renderFooterIdle() {
  ...
  return `
    <div class="footer">
      <button class="primary-btn" data-action="start-round" ${disabled ? "disabled" : ""}>\u041D\u0430\u0447\u0430\u0442\u044C \u0440\u0430\u0443\u043D\u0434</button>
      ${!isGM() ? `<div class="gm-note">\u0420\u0430\u0443\u043D\u0434 \u043C\u043E\u0436\u0435\u0442 \u043D\u0430\u0447\u0430\u0442\u044C \u0442\u043E\u043B\u044C\u043A\u043E \u043C\u0430\u0441\u0442\u0435\u0440.</div>` : ""}
    </div>
  `;
}
function renderRound() {
  ...
  if (isGM()) {
    ...
    rows = pc.length ? pc.map(renderReadyRow).join("") : `<div class="empty">\u041D\u0435\u0442 \u043F\u0435\u0440\u0441\u043E\u043D\u0430\u0436\u0435\u0439, \u0434\u043E\u0441\u0442\u0443\u043F\u043D\u044B\u0445 \u0434\u043B\u044F \u0440\u0435\u0434\u0430\u043A\u0442\u0438\u0440\u043E\u0432\u0430\u043D\u0438\u044F \u0438\u0433\u0440\u043E\u043A\u0430\u043C\u0438.</div>`;
  }
  const ready = allReady(characters, state);
  const statusText = ready ? "\u0412\u0441\u0435 \u0433\u043E\u0442\u043E\u0432\u044B!" : `\u0413\u043E\u0442\u043E\u0432\u044B: ${readyCount(characters, state)} \u0438\u0437 ${characters.length}`;
  return `
    <div class="list">${rows}</div>
    <div class="status-line ${ready ? "is-ready" : ""}">${statusText}</div>
    <div class="footer">
      <button class="primary-btn" data-action="go-battle" ${!isGM() || !ready ? "disabled" : ""}>\u0412 \u0431\u043E\u0439</button>
      ${isGM() ? `<button class="text-btn" data-action="reset">\u0421\u0431\u0440\u043E\u0441\u0438\u0442\u044C \u0440\u0430\u0443\u043D\u0434</button>` : `<div class="gm-note">\u041F\u0435\u0440\u0435\u0445\u043E\u0434 \u0432 \u0431\u043E\u0439 \u2014 \u0440\u0435\u0448\u0435\u043D\u0438\u0435 \u043C\u0430\u0441\u0442\u0435\u0440\u0430.</div>`}
    </div>
  `;
}
function renderReadyRow(item) {
  ...
  const animTs = readyAnimatedAt.get(item.id);
  const isNewlyReady = ready && animTs != null && now - animTs < ANIM_WINDOW_MS;
...
function renderBattle() {
  ...
  const rows = order.map((id, index) => {
    ...
    const value = Number.isFinite(finalInitiative[id]) ? finalInitiative[id] : "\u2014";
    return `
  ...
  const isLast = state.currentIndex === null || state.currentIndex >= order.length - 1;
  return `
    <div class="list">${rows}</div>
    <div class="footer">
      <button class="primary-btn" data-action="next-creature" ${!isGM() ? "disabled" : ""}>${isLast ? "\u0421\u043B\u0435\u0434\u0443\u044E\u0449\u0435\u0435 \u0441\u0443\u0449\u0435\u0441\u0442\u0432\u043E \u2192 \u041D\u0430\u0447\u0430\u0442\u044C \u0440\u0430\u0443\u043D\u0434" : "\u0421\u043B\u0435\u0434\u0443\u044E\u0449\u0435\u0435 \u0441\u0443\u0449\u0435\u0441\u0442\u0432\u043E"}</button>
      ${isGM() ? `<button class="text-btn" data-action="reset">\u0421\u0431\u0440\u043E\u0441\u0438\u0442\u044C \u0431\u043E\u0439</button>` : `<div class="gm-note">\u041E\u0447\u0435\u0440\u0435\u0434\u044C \u0445\u043E\u0434\u0430 \u0432\u0435\u0434\u0451\u0442 \u043C\u0430\u0441\u0442\u0435\u0440.</div>`}
    </div>
  `;
}
...
function attachHandlers() {
  ...
  app.onkeydown = (event) => {
    if (event.key === "Enter" && event.target.dataset && event.target.dataset.action === "set-initiative") {
      event.target.blur();
    }
  };
}
async function init() {
  ...
  lib_default.player.onChange(async (player) => {
    role = player.role;
    await refreshRoleMap();
    render();
  });
  lib_default.party.onChange(async () => {
    await refreshRoleMap();
    render();
  });
