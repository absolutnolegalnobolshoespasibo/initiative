...
async function centerOn(itemId) {
  try {
    const bounds = await lib_default.scene.items.getItemBounds([itemId]);
    if (bounds) {
      await lib_default.viewport.animateToBounds(bounds);
    }
  } catch (e) {
...
function renderReadyRow(item) {
  ...
  const current2 = computeInitiative(item, state, roleMap, ITEM_KEY, Date.now());
  return `
    ...
    item.name
      ...
      <div class="current-init">${current2}</div>
      <input
        type="checkbox"
        class="ready-check"
